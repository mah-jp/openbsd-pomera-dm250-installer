...
3. rkdrm WSDISPLAYIO_SMODE fix (mlterm-fb DUMBFB console)
4. bcmbt 2s delay fix (Bluetooth UART attach)
5. DM250 smart kernel config (OpenBSD (DM250))
...
def find_objdump() -> Optional[str]:
    """
    Locates an objdump binary capable of disassembling ARM binaries.
    Prefers ARM-specific tools, checks '-i' target support, and falls back to LLVM.
    """
    candidates = [
    ...
    for tool in candidates:
        ...
        if not path:
            continue
        # Verify that objdump supports arm/elf32-littlearm or is llvm-objdump
        code, out = run_cmd([path, '-i'])
        if code == 0:
            if 'arm' in out.lower() or 'elf32-littlearm' in out.lower():
                return path
        else:
            code_v, out_v = run_cmd([path, '--version'])
            if 'llvm' in out_v.lower():
                return path
    return None
...
def find_nm() -> Optional[str]:
    """
    Locates an nm binary capable of inspecting symbols in ARM ELF kernels.
    """
    candidates = [
...
def disassemble_symbol(objdump_bin: str, kernel_path: str, symbol: str) -> Tuple[int, str]:
    """
    Disassembles a specific symbol using either GNU or LLVM objdump.
    Handles option syntax differences between GNU (--disassemble=<sym>)
    and LLVM (--disassemble-symbols=<sym> / --disassemble-symbol=<sym>).
    """
    code, out = run_cmd([objdump_bin, '--version'])
    is_llvm = 'LLVM' in out or 'llvm' in os.path.basename(objdump_bin).lower()

    if is_llvm:
        cmd = [objdump_bin, '-d', f'--disassemble-symbols={symbol}', kernel_path]
        c, o = run_cmd(cmd)
        if c == 0:
            return c, o
        cmd = [objdump_bin, '-d', f'--disassemble-symbol={symbol}', kernel_path]
        c, o = run_cmd(cmd)
        if c == 0:
            return c, o

    # GNU objdump syntax: --disassemble=<sym>
    cmd = [objdump_bin, '-d', f'--disassemble={symbol}', kernel_path]
    c, o = run_cmd(cmd)
    if c == 0:
        return c, o

    # Fallback to LLVM syntax if GNU syntax failed
    if not is_llvm:
        cmd = [objdump_bin, '-d', f'--disassemble-symbols={symbol}', kernel_path]
        c, o = run_cmd(cmd)
        if c == 0:
...
def check_x11_key_patch(kernel_path: str, objdump_bin: Optional[str], nm_bin: Optional[str]) -> Tuple[bool, str]:
    ...
    """
    if objdump_bin:
        code, out = disassemble_symbol(objdump_bin, kernel_path, 'gpiokeys_console_key')
...
def check_usb_hub_patch(kernel_path: str, objdump_bin: Optional[str]) -> Tuple[bool, str]:
    ...
    if objdump_bin:
        # Check dwc2_hc_intr
        code, out = disassemble_symbol(objdump_bin, kernel_path, 'dwc2_hc_intr')
        if code == 0 and 'dwc2_hc_intr' in out:
            has_stack_smash = 'stack_smash' in out
            has_boundary_check = bool(re.search(r'\bcmp\s+\w+,\s*#(15|0xf)\b', out))
            if has_stack_smash or has_boundary_check:
                ...
                return True, f'dwc2_hc_intr: {joined_reasons}'
        ...
        code_n, out_n = disassemble_symbol(objdump_bin, kernel_path, 'dwc2_hc_n_intr')
...
def check_smode_patch(kernel_path: str, objdump_bin: Optional[str]) -> Tuple[bool, str]:
    ...
    """
    if objdump_bin:
        code, out = disassemble_symbol(objdump_bin, kernel_path, 'rkdrm_wsioctl')
...
def check_bt_patch(kernel_path: str, objdump_bin: Optional[str]) -> Tuple[bool, str]:
    ...
    """
    if objdump_bin:
        code, out = disassemble_symbol(objdump_bin, kernel_path, 'bcmbt_load_firmware')
